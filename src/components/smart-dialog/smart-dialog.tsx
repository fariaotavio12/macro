...
export const SmartOverlay = ({
	...
	return (
		<Dialog open={open} onOpenChange={onOpenChange}>
			...
			>
				{hasHeader ? (
					<div className="flex shrink-0 items-start justify-between gap-3 border-b px-5 pb-4 pt-5">
						<div className="flex min-w-0 items-start gap-2">
							{headerIcon && (
								<span className="text-muted-foreground mt-0.5 shrink-0 [&>svg]:size-5">
									{headerIcon}
								</span>
							)}
					...
					<>
						...
						<DialogClose asChild>
							<Button variant="ghost" size="icon-sm" className="absolute right-3 top-3 z-10">
								<XIcon size={16} strokeWidth={1.5} />
								<span className="sr-only">Fechar</span>
							</Button>
						</DialogClose>
				...
				<div className={cn("min-h-0 flex-1 overflow-y-auto", hasHeader && "px-5 py-4")}>
					{children}
				</div>
