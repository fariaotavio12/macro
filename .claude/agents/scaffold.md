...
For a very small one-screen feature, `src/features/<feature>/page-<feature>.tsx` is acceptable.
...
The project uses i18n (single `translation` namespace, co-located files). For any user-facing copy in a new feature, create `src/features/<feature>/translations/{pt,en}.json` (wrapped under the feature key), register it in `src/app/i18n/resources.ts`, and use `const { t } = useT()` with flat keys (`t("<feature>.<path>")`). No hardcoded strings. See the `i18n-screens` skill.
