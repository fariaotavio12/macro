...
- New/touched domain code belongs in `src/features/<feature>`.
- Domain API/types should be in `src/features/<feature>/api`, not legacy `src/api/services` or `src/api/types/<domain>`.
- Feature state belongs in `src/features/<feature>/model`.
...
- Do NOT use raw `<Table>/<TableHeader>/<TableBody>/<TableRow>/<TableCell>` with manual mapping. Use `TableCustom` compound component.
...
- Feature-local state in `src/features/<feature>/model`.
